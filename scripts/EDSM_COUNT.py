...
import numpy as np
from TraversedState import *

def EDSM_Count(dfa, positive, negative):
	tp = np.zeros((len(dfa.FiniteSetOfStates)))
	tn = np.zeros((len(dfa.FiniteSetOfStates)))
	sc = 0
	
	for s in positive:
		qt = traversedState(s, dfa)
		if qt != 0:
			tp[qt-1] += 1
	for s in negative:
		qt = traversedState(s,dfa)
		if qt != 0:
			tn[qt-1] += 1
	for st in dfa.FiniteSetOfStates:
		if sc != float("-inf"):
			if tn[st-1] > 0:
				if tn[st-1] > 0:
					sc = float("-inf")
